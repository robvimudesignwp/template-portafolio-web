...
$sendto = 'robert-870519@hotmail.com';
...
if ( !empty( $_POST ) ) {
    ...
    if (empty( $name ) || empty( $from ) || empty( $message ) ) {
    	echo json_decode( array('status' => 0, 'message' => 'A require field is missing' ) );
    ...
    echo json_encode( array('status' => 0, 'message' => 'Email not send sucessfully. Please try again' ) );
}
